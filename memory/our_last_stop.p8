...
function _update()
	cls()
	t += 1/30
	ti = ti + 1/30 + rnd(1/60) - 1/120

	local padding = 0.2
	local sy = -t*8
	sy = sy % padding - padding

	local L1,L2 = {},{}
	for y=sy,128+padding,padding do
		local x = 16 
			+ abs(4*cos(sin(t/3 + y/100 - 0.1*cos(t/8))/(1*cos(t/8)+5)) 
			+ 8*cos(y/200))
			+ abs(4*sin(sin(y/200 - t/18)))
			- abs(12*cos(y/300 + t/64 - ti/4)-4)
			add(L1, vec(64+x,y))
			add(L2, vec(64-x,y))
			pset(64+x,y,7)
			pset(64-x,y,7)
			pset(64+x,y+6+4*sin(t/3+y/64),5)
			pset(64-x,y+6+4*sin(t/3+y/64),5)
	end
	--polyp(L1,vec(),7)
	--polyp(L2,vec(),7)
