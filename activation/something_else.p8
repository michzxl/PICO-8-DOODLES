...
function _init()
	...
	side = 7
	total_size = 100
	cube_size = total_size / side

	cf,cv = poly_cube(total_size)
...
function _update()
	...
	if t>15 and t<22 then
		...
		for i=1,500 do
			local ang,r = rnd(1),sqrt(rnd(64*64*2))
			local ang2,r2 = ang,r-1.5

			local x,y = r*cos(ang)+64,r*sin(ang)+64
	...
	local x_start, x_end, x_step, y_start, y_end, y_step, z_start, z_end, z_step = get_dirs(ux,uy,uz)

	local rcv = {}
	for v in all(cv) do
		local nv = ux*v.x + uy*v.y + uz*v.z
		add(rcv,nv)
	end
	...
	for f in all(cf) do
		local fv = {}
		for i in all(f) do
			add(fv, rcv[i] * (0.5*sin(t/8) + 1))
		end
		if poly_normal(fv).z<0 then
			if t<6 then 
				polyf(fv,vec(64,64),7) 
				polyv(fv,vec(64,60),6)
			else
				polyv(fv,vec(64,60),7)
			end
		end
	end
	...
	for x=0,side-1 do
		...
		for y=0,side-1 do
			...
			for z=0,side-2 do
				...
				local v = vec(px,py,pz) - vec(total_size/2,total_size/2,total_size/2)

				v = v * (0.5*sin(t/8) + 1)

				v = ux*v.x + uy*v.y + uz*v.z + cen_ofs

				local c = v.z/16 + t/8
				c = c%7+8

				local w = max(0.1,9*sin(v.y/64 + v.x/64 + v.z/200 + t/4))

				if (w>0) then 
					rectfill(v.x,v.y+w,v.x+w,v.y+w+1,c-8)
					rectfill(v.x,v.y,v.x+w,v.y+w,c)
				end
			end
		end
	end
...
function get_dirs(ux,uy,uz)
	local x_forward = ux.z>0
	local y_forward = uy.z>0
	local z_forward = uz.z>0
	...
	return x_start, x_end, x_step, y_start, y_end, y_step, z_start, z_end, z_step
end
