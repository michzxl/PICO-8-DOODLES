...
function _draw()
	...
	fillp(FILLS2[flr((t*4)%#FILLS2+1)])
	for i=1,500 do
		
		local x,y = rnd(128),rnd(128)
		fillp(FILLS2[flr((t/8 + x/128 + (y+16*sin(y/32+x/32))/128)%#FILLS2+1)])
		local c = pget(x,y)
		if (c~=0) then circ(x,y+2,1,c) end
	end
	fillp()
...
function POLY.objdraw(obj, trans, rot, scale, wire, col)
	...
	for f in all(fs) do
		...
		if n.z>=0 then
			...
			local light = (-(n.y - n.z*0.5 + n.x*1)+2.5/2)/3
			local col = light*(ramp)
			local ld = col - flr(col)
			local fill = FILLS[flr(ld*#FILLS+1)]
			local fcol = mid(0,ramp-1,flr(col))
				+ 16*mid(0,ramp-1,flr(col + 1))
