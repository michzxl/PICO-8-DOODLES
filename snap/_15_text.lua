function draw_text(str,x,y,sw,sh,ang,col)
	...
	local z = (rnd(1)<0.05 and 2 or 1)

	local ln = 0
	local cl = 0
	for i=1,#str do
		if i%2==1 and stat(1)>0.90 then return end
		...
		if char=="\n" then
			...
			local rect = {
				vec(cl*8, ln*8),
				vec(cl*8 + 8*sw*z, ln*8),
				vec(cl*8 + 8*sw*z, ln*8 + 8*sh*z),
				vec(cl*8, ln*8 + 8*sh*z),
			}

			for _,v in pairs(rect) do
				v:set(R1*v.x + R2*v.y + vec(rnd(2)-1,rnd(2)-1) + vec(1*cos(t/8+i/8+v.x/64),1*sin(t/8+i/8+v.y/64)))
			end

			for _,v in pairs(rect) do
				v:set(v + pos + vec(4*cos(t/8+i/12+pos.x/64)))
			end
