...
function _update()
	...
	local us = vec(-t/8+orot, t/12\0.1*0.1, (t/12+1)\0.1*0.1):u_rot_yxz()
	local scale = vec.one() * (32 + rnd(4)*sin(t/8))
	...
	for f in all(tet.f) do
		local buf = {}
		for vi in all(f) do
			add(buf, (vs[vi]-vec(64,64))*2.0+vec(64,64))
		end
		poke(0x5f5e,g())
	...
	for f in all(tet.f) do
		local buf = {}
		for vi in all(f) do
			add(buf, (vs[vi]-vec(64,64))*1.5+vec(64,64))
		end
		poke(0x5f5e,g())
		--line(buf[1].x,buf[1].y,buf[2].x,buf[2].y,15)
