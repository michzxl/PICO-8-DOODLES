...
function os2d_eval(x,y)
	...
	if at1>0 then
		...
		val=val+at1*at1*(_os2d_grd[i]*dx1+_os2d_grd[i+1]*dy1)
	end
	...
	if at2>0 then
		...
		val=val+at2*at2*(_os2d_grd[i]*dx2+_os2d_grd[i+1]*dy2)
	end
	...
	if at0>0 then
		...
		val=val+at0*at0*(_os2d_grd[i]*dx0+_os2d_grd[i+1]*dy0)
	end
	...
	if atx>0 then
		...
		val=val+atx*atx*(_os2d_grd[i]*dx_ext+_os2d_grd[i+1]*dy_ext)
	end
