...
function rprint(s,dx,dy,a,w,p)
	...
	for i=1,#s do
		...
		elseif char==' ' then
			...
			pal(7,4,0)
			rectfill(xf,yf+4,xf+7,yf+7,3)
			rectfill(xf,yf+7,xf+7,yf+7,8)
			sspr(sx,sy,8,8,xf,yf-1)
			sspr(sx,sy,8,8,xf+1,yf)
			sspr(sx,sy,8,8,xf-1,yf)

			pal(7,0,0)
			sspr(sx,sy,8,8,xf,yf,i%10==wider%10 and 16 or 8,i%10==wider%10 and 8 or 8)
			if rnd(1)<0.025 then
				x += 1
				sspr(sx,sy,8,8,xf+1,yf-2,i%10==wider%10 and 16 or 8,i%10==wider%10 and 8 or 8)
			end

			pal(7,7,0)

			if i%10==wider%10 then
				x+=w*16
			else
				x+=w*8
			end
