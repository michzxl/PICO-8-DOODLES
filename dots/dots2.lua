...
thrower = {
	...
	grabstone = function(self)
		...
		stn:set(
			self.override_pool and vec(x,y) or stn.pos,
			vec.frompolar(fact.ang + rnd_spread(fact.ang_spread),fact.spd_init + rnd_spread(fact.spd_spread)),
			self.gravity,
			self.damp,
			fact.killspd,
			fact.size_init + rnd_spread(fact.size_spread),
			fact.life + rnd_spread(fact.life_spread),
			self.override_pool and fact.col or stn.col
		)
		stn.updoverride = fact.updoverride
		if stn.updoverride then 
			stn.upd = function(self)
				self:updoverride() 
...
stone = {
	...
	upd = function(self)
		...
		vel:set(vel * self.damp + self.grav)
		pos:set(pos + vel)
		self.life -= 1
